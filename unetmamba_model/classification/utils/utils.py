...
def load_checkpoint_ema(config, model, optimizer, lr_scheduler, loss_scaler, logger, model_ema: ModelEma=None):
    ...
    if 'unetmamba' in checkpoint:
        msg = model.load_state_dict(checkpoint['unetmamba'], strict=False)
        logger.info(f"resuming unetmamba: {msg}")
    else:
        logger.warning(f"No 'unetmamba' found in {config.MODEL.RESUME}! ")
...
def load_pretrained_ema(config, model, logger, model_ema: ModelEma=None):
    ...
    if 'unetmamba' in checkpoint:
        msg = model.load_state_dict(checkpoint['unetmamba'], strict=False)
        logger.warning(msg)
        logger.info(f"=> loaded 'unetmamba' successfully from '{config.MODEL.PRETRAINED}'")
    else:
        logger.warning(f"No 'unetmamba' found in {config.MODEL.PRETRAINED}! ")

    if model_ema is not None:
        if "model_ema" in checkpoint:
            logger.info(f"=> loading 'model_ema' separately...")
        key = "model_ema" if ("model_ema" in checkpoint) else "unetmamba"
...
def save_checkpoint_ema(config, epoch, model, max_accuracy, optimizer, lr_scheduler, loss_scaler, logger, model_ema: ModelEma=None, max_accuracy_ema=None):
    save_state = {'unetmamba': model.state_dict(),
